...
import {
  ...
  Users, Landmark, TableProperties, Settings,
  type LucideIcon,
} from 'lucide-react'
import ThemeToggle from './ThemeToggle'

interface DockLink { to: string; icon: LucideIcon; label: string; center?: boolean }
...
const links: DockLink[] = [
  ...
  { to: '/budget', icon: TableProperties, label: 'Бюджет' },
  { to: '/settings', icon: Settings, label: 'Ещё' },
]

const DockItem = ({ to, icon: Icon, label, center }: DockLink) => (
  <NavLink to={to} end={to === '/'} className={({ isActive }) => clsx(
    ...
    background: isActive ? 'color-mix(in srgb, var(--surface-overlay) 82%, transparent)' : 'transparent',
