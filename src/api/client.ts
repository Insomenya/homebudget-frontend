...
const api = {
  ...
  lookups: {
    list: (group?: string) =>
      request<LookupValue[]>('GET', `/lookups${group ? `?group=${group}` : ''}`),
    create: (d: CreateLookupInput) => request<LookupValue>('POST', '/lookups', d),
    update: (id: number, d: UpdateLookupInput) =>
      request<LookupValue>('PUT', `/lookups/${id}`, d),
    delete: (id: number) => request<void>('DELETE', `/lookups/${id}`),
  },

  members: {
    list: (inclArch = false) =>
      request<Member[]>('GET', `/members?include_archived=${inclArch}`),
    get: (id: number) => request<Member>('GET', `/members/${id}`),
    create: (d: CreateMemberInput) => request<Member>('POST', '/members', d),
    update: (id: number, d: UpdateMemberInput) =>
      request<Member>('PUT', `/members/${id}`, d),
  ...
  accounts: {
    ...
    update: (id: number, d: UpdateAccountInput) =>
      request<Account>('PUT', `/accounts/${id}`, d),
    delete: (id: number) => request<void>('DELETE', `/accounts/${id}`),
  },

  categories: {
    list: (inclArch = false) =>
      request<Category[]>('GET', `/categories?include_archived=${inclArch}`),
    get: (id: number) => request<Category>('GET', `/categories/${id}`),
    create: (d: CreateCategoryInput) => request<Category>('POST', '/categories', d),
    update: (id: number, d: UpdateCategoryInput) =>
      request<Category>('PUT', `/categories/${id}`, d),
    delete: (id: number) => request<void>('DELETE', `/categories/${id}`),
  },

  groups: {
    list: (inclArch = false) =>
      request<SharedGroup[]>('GET', `/groups?include_archived=${inclArch}`),
    get: (id: number) => request<SharedGroup>('GET', `/groups/${id}`),
    create: (d: CreateSharedGroupInput) =>
      request<SharedGroup>('POST', '/groups', d),
    update: (id: number, d: UpdateSharedGroupInput) =>
      request<SharedGroup>('PUT', `/groups/${id}`, d),
  ...
  transactions: {
    list: (params: QsParams = {}) =>
      request<TransactionList>('GET', `/transactions${qs(params)}`),
    get: (id: number) => request<Transaction>('GET', `/transactions/${id}`),
    create: (d: CreateTransactionInput) =>
      request<Transaction>('POST', '/transactions', d),
  ...
  planned: {
    list: (all = false) =>
      request<PlannedTransaction[]>('GET', `/planned?all=${all}`),
    get: (id: number) =>
      request<PlannedTransaction>('GET', `/planned/${id}`),
    create: (d: CreatePlannedInput) =>
      request<PlannedTransaction>('POST', '/planned', d),
    update: (id: number, d: UpdatePlannedInput) =>
      request<PlannedTransaction>('PUT', `/planned/${id}`, d),
  ...
  loans: {
    list: (all = false) =>
      request<Loan[]>('GET', `/loans${all ? '?all=true' : ''}`),
    get: (id: number) => request<Loan>('GET', `/loans/${id}`),
    create: (d: CreateLoanInput) => request<Loan>('POST', '/loans', d),
    update: (id: number, d: UpdateLoanInput) =>
      request<Loan>('PUT', `/loans/${id}`, d),
