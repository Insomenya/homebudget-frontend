...
const FluidCanvas = ({ m0, m1, ratio }: FluidCanvasProps) => {
  ...
  useEffect(() => {
    ...
    simRef.current = createFluidSimulation(
      canvas,
      [
        { fill: m0.color, stroke: m0.colorLight },
        { fill: m1.color, stroke: m1.colorLight },
      ],
      ratio,
    )
...
const BattleWidget = ({ data: dashData, onRemove }: WidgetComponentProps) => {
  const firstGroupId = dashData?.settlements?.[0]?.group_id ?? null
    ...
    >
      <div
        ...
        style={{
          ...
          background:
            'linear-gradient(180deg, color-mix(in srgb, var(--surface-overlay) 88%, transparent), color-mix(in srgb, var(--surface-elevated) 92%, transparent))',
        }}
      >
        {hasBoth && m0 && m1 && (
          <FluidCanvas m0={m0} m1={m1} ratio={ratio} />
        )}
        ...
        <div
          ...
          style={{
            background:
              'linear-gradient(180deg, rgba(0,0,0,0.12), rgba(0,0,0,0.02) 18%, rgba(0,0,0,0.08) 100%)',
          }}
        />
            ...
            <div className="absolute top-4 left-4 right-4 z-10 grid grid-cols-[1fr_auto_1fr] gap-3 items-start pointer-events-none">
              <OverlayPill align="left">
                <div className="flex items-center gap-2 mb-1">
                  ...
                  <span className="text-sm font-semibold" style={{ color: m0.colorLight }}>
                    {m0.name}
                  </span>
                </div>
                <div className="text-2xl font-black tabular-nums app-text">
                  {m0.percentage.toFixed(1)}%
                </div>
                <div className="mt-1 text-[11px] app-text-secondary">
                  Оплатил {formatRub(m0.paid)}
                </div>
              </OverlayPill>
              ...
              <OverlayPill align="center">
                ...
                <div className="text-[11px] app-text-muted">
                  Всего {battle ? formatRub(battle.totalExpenses) : '—'}
                </div>
              </OverlayPill>

              <OverlayPill align="right">
                <div className="flex items-center justify-end gap-2 mb-1">
                  <span className="text-sm font-semibold" style={{ color: m1.colorLight }}>
                    {m1.name}
                  </span>
                  <span className="text-lg">{m1.icon}</span>
                </div>
                <div className="text-2xl font-black tabular-nums app-text">
                  {m1.percentage.toFixed(1)}%
                </div>
                <div className="mt-1 text-[11px] app-text-secondary">
                  Оплатил {formatRub(m1.paid)}
                </div>
              </OverlayPill>
            </div>
                  ...
                  {battle.debts.map((d, i) => (
                    <OverlayPill key={i} align="center">
                      <div className="text-xs app-text-secondary">
                        ...
                        <span className="ml-2 font-bold app-negative tabular-nums">
                          {formatRub(d.amount)}
                        </span>
                      </div>
                    </OverlayPill>
