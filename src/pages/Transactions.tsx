...
import type { TransactionList, TxType, Account, Category, Loan, SharedGroup, Member, CreateTransactionInput, Transaction } from '../types'

const typeBadge: Record<TxType, 'danger' | 'success' | 'neutral'> = { expense: 'danger', income: 'success', transfer: 'neutral' }

interface Filters { page: number; limit: number; sort: string; dir: string; search: string; from: string; to: string; type: string }
...
const AddTxModal = ({ open, onClose, onCreated }: { open: boolean; onClose: () => void; onCreated: () => void }) => {
  const { label } = useMeta()
  const [form, setForm] = useState<TxForm>({
    date: new Date().toISOString().split('T')[0], amount: '', description: '', type: 'expense',
    account_id: '', category_id: '', loan_id: '', shared_group_id: '', paid_by_member_id: '',
  })
  ...
  const handleSubmit = async (e: FormEvent) => {
    e.preventDefault()
    await create({
      date: form.date, amount: parseFloat(form.amount) || 0, description: form.description, type: form.type,
    ...
    <Modal open={open} onClose={onClose} title="Новая операция">
      <form onSubmit={handleSubmit} className="flex flex-col gap-4">
        <div className="grid grid-cols-2 gap-4">
          ...
          <Input label="Сумма" type="number" step="0.01" min="0.01" value={form.amount} placeholder="0.00" onChange={(e) => setForm({ ...form, amount: e.target.value })} />
        </div>
        <div className="grid grid-cols-2 gap-4">
          <Select label="Категория" value={form.category_id} onChange={(e) => setForm({ ...form, category_id: e.target.value })}>
            <option value="">—</option>
            {groupedCats.map((c) => <option key={c.id} value={c.id}>{c.parent_id ? '   ' : ''}{c.icon} {c.name}</option>)}
          </Select>
        ...
        <Input label="Описание" value={form.description} placeholder="Продукты, такси…" onChange={(e) => setForm({ ...form, description: e.target.value })} />
        <div className="grid grid-cols-2 gap-4">
          <Select label="Счёт" value={form.account_id} onChange={(e) => setForm({ ...form, account_id: e.target.value })}>
            <option value="">—</option>
            {(accs ?? []).map((a) => <option key={a.id} value={a.id}>{a.name}</option>)}
          </Select>
          <Select label="Кредит" value={form.loan_id} onChange={(e) => setForm({ ...form, loan_id: e.target.value })}>
...
const Transactions = () => {
  ...
  const { run: updateTx } = useMutation((args: { id: number; tx: Transaction; field: string; value: string }) => {
    const t = args.tx
      ...
      <Card className="mb-4">
        <CardBody>
          <div className="flex flex-wrap gap-3">
            ...
            <Input type="date" value={f.from} onChange={(e) => setF((p) => ({ ...p, from: e.target.value, page: 1 }))} className="w-40" />
            <Input type="date" value={f.to} onChange={(e) => setF((p) => ({ ...p, to: e.target.value, page: 1 }))} className="w-40" />
